...
for neurons in hidden_layers_list:
    nn = NN_classifier(neurons,10,784,100,0.1,"relu",(5e-5),"normal", 'Cross-Entropy')
    start = time.time()
...
with open(neural.out_path+"/f.txt", "w+") as file1:

    for i in range(len(train_acc_adapt)):
        file1.write(f"Train accuracy for hidden layers of {hidden_layers_list[i]} units is: {train_acc_adapt[i]}% \n")
    file1.write("\n")
    for i in range(len(test_acc_adapt)):
        file1.write(f"Test accuracy for hidden layers of {hidden_layers_list[i]} units is: {test_acc_adapt[i]}% \n")
    file1.write("\n")
    for i in range(len(training_time_adapt)):
        file1.write(f"Training time for hidden layers of {hidden_layers_list[i]} units is: {training_time_adapt[i]} sec \n")
    file1.write("\n")
    for i in range(len(epoch_list_adapt)):
        file1.write(f"Total no. of epochs for hidden layers of {hidden_layers_list[i]} units is: {epoch_list_adapt[i]} \n")
    file1.write("\n")
