...
hidden_layers_list = [[50,50],[50,50,50],[50,50,50,50],[50,50,50,50,50]]
...
with open(neural.out_path+"/e.txt", "w+") as file1:
    file1.write(f"result with sigmoid Activation Units.........\n")
    ...
    for i in range(len(training_time_adapt)):
        file1.write(f"Training time for hidden layer of {hidden_layers_list[i]} units is: {training_time_adapt[i]} sec \n")
    file1.write("\n")
...
hidden_layers_list_Len = []
for i in hidden_layers_list:
    hidden_layers_list_Len.append(len(i))


fig1 = plt.figure()
plt.plot(hidden_layers_list_Len,train_acc_adapt,label='Train Accuracies')
plt.plot(hidden_layers_list_Len,test_acc_adapt,label = 'Test Accuracies')
plt.xlabel('Hidden Layers length')
plt.ylabel('Accuracy in %')
plt.legend()
plt.savefig(neural.out_path+'/q2e_accuracies_sigmoid.jpg')
...
for neurons in hidden_layers_list:
    nn = NN_classifier(neurons,10,784,100,0.1,"relu",(5e-5),"normal", 'MSE')
    start = time.time()
    nn.init_NN(train[:,:-1]/255,train[:,-1])
    end = time.time()
    training_time_adapt.append(end-start)
...
with open(neural.out_path+"/e.txt", "a") as file1:
    file1.write(f"result with ReLu Activation Units.........\n")
...
plt.ylabel('Accuracy in %')
plt.legend()
plt.savefig(neural.out_path+'/q2e_accuracies_relu.jpg')