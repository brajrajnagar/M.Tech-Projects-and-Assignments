...
class NN_classifier:
    ...
    def init_NN(self,train_X,train_Y):
        ...
        while not_converged:
            ...
            # print(abs(initial_cost-final_cost))s
...
for neurons in hidden_layers_list:
    nn = NN_classifier([neurons],10,784,100,0.1,"sigmoid",(5e-5),"normal", 'MSE')
    start = time.time()
    nn.init_NN(train[:,:-1]/255,train[:,-1])
    end = time.time()
    training_times.append(end-start)
    epoch_list.append(nn.epoch)
    predictedTrain = nn.class_prediction(train[:,:-1]/255)
    predictedTest = nn.class_prediction(test[:,:-1]/255)
    train_accuracies.append((Accuracy(train[:,-1],predictedTrain)*100))
    test_accuracies.append((Accuracy(test[:,-1],predictedTest)*100))
    ...
    cm_display.plot()
    plt.savefig(neural.out_path+'/q2b_'+str(neurons)+'.jpg')


# Writing outputs
with open(neural.out_path+"/b.txt", "w+") as file1:
    for i in range(len(train_accuracies)):
        file1.write(f"Train accuracy for hidden layer of {hidden_layers_list[i]} units is: {train_accuracies[i]}%\n")
    file1.write("\n")
    for i in range(len(test_accuracies)):
        file1.write(f"Test accuracy for hidden layer of {hidden_layers_list[i]} units is: {test_accuracies[i]}%\n")
    file1.write("\n")
    for i in range(len(training_times)):
        file1.write(f"Training time for hidden layer of {hidden_layers_list[i]} units is: {training_times[i]} sec\n")
    file1.write("\n")
    for i in range(len(epoch_list)):
        file1.write(f"Total no. of epochs for hidden layer of {hidden_layers_list[i]} units is: {epoch_list[i]}\n")
    file1.write("\n")


fig1 = plt.figure()
plt.plot(hidden_layers_list,train_accuracies,label='Train Accuracies')
plt.plot(hidden_layers_list,test_accuracies,label = 'Test Accuracies')
plt.xlabel('Number of Hidden Layer Units')
plt.ylabel('Accuracy in %')
plt.legend()
plt.savefig(neural.out_path+'/q2b_accuracies.jpg')


fig2 = plt.figure()
plt.plot(hidden_layers_list, training_times, label='Training Times')
plt.xlabel('Number of Hidden Layer Units')
plt.ylabel('Training Time (in secs)')
plt.legend()
plt.savefig(neural.out_path+'/q2b_traintime.jpg')