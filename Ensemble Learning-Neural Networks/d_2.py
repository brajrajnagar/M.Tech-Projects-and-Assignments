...
for neurons in hidden_layers_list:
    ...
    cm_display.plot()
    plt.savefig(neural.out_path+'/q2d_cm_sigmoid.jpg')



with open(neural.out_path+"/d.txt", "w+") as file1:
    file1.write("result with sigmoid Activation Units.........\n")
...
for neurons in hidden_layers_list:
    nn = NN_classifier(neurons,10,784,100,0.1,"relu",(5e-5),"adaptive", 'MSE')
    start = time.time()
    nn.init_NN(train[:,:-1]/255,train[:,-1])
    end = time.time()
    training_time_adapt.append(end-start)
    ...
    cm_display = met.ConfusionMatrixDisplay(confusion_matrix = confusion_matrix)
    cm_display.plot()
    plt.savefig(neural.out_path+'/q2d_cm_relu.jpg')





with open(neural.out_path+"/d.txt", "a") as file1:
    file1.write("result with ReLu Activation Units.........\n")
